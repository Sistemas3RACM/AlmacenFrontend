<template>
    <div class="row" v-if="show">
        <div class="col-md-12" v-if="salidas.length > 0">
            <table class="table table-striped justify-content-center text-center">
                ...
                <tbody>
                    <tr v-for="salida in paginated" :key="salida.idSolicitud">
                        <td>{{ salida.numeroDeSerie }}</td>
                        <td>{{ salida.solicitante }}</td>
                        <td>{{ salida.fechaSolicitud.slice(0, 10) }}</td>
                        <td>{{ salida.proyectoCuenta }}</td>
                        <!-- <td class="button-container">

                            <button @click="abrirModalInformacion(salida)" class="btn btn-primary btn-redondo">
        ...
        <div class="col-md-9" v-else>
            <h1 class="text-center">No hay ninguna salida registrada.</h1>
        </div>
...
export default {
    ...
    computed: {
        ...
        // `Math.ceil()` function.
        totalPages() {
            if (!this.salidas) return 0;
            return Math.ceil(this.salidas.length / this.pageSize);
        },
        // The above code is a method in a Vue component that is used for pagination.
        paginated() {
            if (!this.salidas) return null;

            const sortedsalidas = this.salidas.slice().reverse();

            const startIndex = (this.currentPage - 1) * this.pageSize;
            const endIndex = startIndex + this.pageSize;

            return sortedsalidas.slice(startIndex, endIndex);
        },
    },
    methods: {
        goToPage(page) {
